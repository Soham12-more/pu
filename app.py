import tkinter as tk # Import the tkinter library for GUI development and interfaces 
import customtkinter as ck # Import customtkinter for enhanced tkinter widgets
import pandas as pd # Import pandas for data manipulation and analysis
import numpy as np # Import numpy for numerical operations
import pickle # Import pickle for object serialization and deserialization
import mediapipe as mp # Import mediapipe for machine learning solutions in computer vision
import cv2 # Import OpenCV for computer vision tasks
from PIL import Image, ImageTk # Import PIL for image processing and handling

from landmarks_list import landmarks
...
def detect(): 
    ...
    if not ret:
        ...
        lmain.after(10, detect) # Keep the loop running to try again
        return
    ...
    if results.pose_landmarks:
        try: 
            ...
            # This line will print the program's state on EVERY frame
            print(f"STATE: {current_stage} | PREDICTION: {bodylang_class} | CONFIDENCE: {prob:.2f}")

            # (You can lower this 0.7 threshold if you need to)
            # Use the CORRECT class names and a LOWER threshold
            if bodylang_class == "floor" and prob > 0.4: 
                current_stage = "floor"
                print("**** STATE SET TO FLOOR ****")
                
            elif current_stage == "floor" and bodylang_class == "lift" and prob > 0.4:
                current_stage = "lift" 
                counter += 1
